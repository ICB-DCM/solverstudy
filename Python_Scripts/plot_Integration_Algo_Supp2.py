...
for atol, rtol in ATOL_RTOLS:
    # AM algorithm
    f_am = f"atol_{atol}__rtol_{rtol}__linSol_6__nonlinSol_2__solAlg_1.tsv"
    df_am = pd.read_csv(
    ...
    # BDF algorithm
    f_bdf = f"atol_{atol}__rtol_{rtol}__linSol_6__nonlinSol_2__solAlg_2.tsv"
    df_bdf = pd.read_csv(
...
plt.savefig(os.path.join(DIR_FIGURES, "Integration_Algo_Supp2.pdf"))
plt.savefig(os.path.join(DIR_FIGURES, "Integration_Algo_Supp2.png"),
            dpi=300)
plt.savefig(os.path.join(DIR_FIGURES, "Integration_Algo_Supp2.eps"),
            dpi=300, format="eps")
