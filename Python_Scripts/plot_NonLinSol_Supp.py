...
def plot_it(solalg, ax1, ax2, letter_1, letter_2):
    linsol = '9'  # KLU
    ...
    for nonlinsol in NONLINSOL_DCT:
        times_for_nonlinsol[nonlinsol] = \
            np.asarray(times_for_nonlinsol[nonlinsol])
        states_for_nonlinsol[nonlinsol] = \
            np.asarray(states_for_nonlinsol[nonlinsol])
    ...
    for nonlinsol in NONLINSOL_DCT:
        # Value scatter plot
        ax.scatter(states_for_nonlinsol[nonlinsol],
                   times_for_nonlinsol[nonlinsol],
    ...
    # Axis bounds
    xmin = min(np.nanmin(states_for_nonlinsol[nonlinsol])
               for nonlinsol in NONLINSOL_DCT)
    xmax = max(np.nanmax(states_for_nonlinsol[nonlinsol])
               for nonlinsol in NONLINSOL_DCT)
    ax.set_xlim([xmin/1.3, xmax*1.3])
    ymin = min(np.nanmin(times_for_nonlinsol[nonlinsol])
               for nonlinsol in NONLINSOL_DCT)
    ymax = max(np.nanmax(times_for_nonlinsol[nonlinsol])
               for nonlinsol in NONLINSOL_DCT)
    ax.set_ylim([ymin * 0.5, ymax*2])

    # Plot text 'A'
    ax.text(-0.13, 1, letter_1, fontsize=labelsize + 5, transform=ax.transAxes)
    ...
    for i_tol, (a_label, r_label) in enumerate(zip(a_labels, r_labels)):
        ...
        ax.text((i_tol+0.5) / (n_tol), -0.13, r_label, fontsize=labelsize,
                transform=ax.transAxes,
                horizontalalignment='center', verticalalignment='bottom')
    ax.text(-0.13, -0.08, 'Abs. tol.:', fontsize=labelsize,
            transform=ax.transAxes, verticalalignment='bottom')
    ax.text(-0.13, -0.13, 'Rel. tol.:',  fontsize=labelsize,
            transform=ax.transAxes, verticalalignment='bottom')
